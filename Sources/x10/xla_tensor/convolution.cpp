...
namespace {
// clang-format off
/* Lower convolution & its backward to XLA
 ...
 *     - BuildGradBias
 *
 * Here're detailed steps from a 4D input to inputs calling into
 * ConvGeneralDilated (the most general conv op in XLA).
 * S4TF input format & shape:
 *   - input: [N, Cin, Hin, Win]
 ...
 * instead of user code convention [N, C, H, W].
 ...
 * We try to use these shapes to explain what happens from a tensor to call
 * into conv op.
 *
 ...
 * functions.
 *
 * Since user code passes the weight with shape [Hker, Wker, 1, Cout] in
...
xla::XlaOp BuildConvBackwardWeight(
  ...
  // of S4TF. The original result of the convolution has the spatial and
