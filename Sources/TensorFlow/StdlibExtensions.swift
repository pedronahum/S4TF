...
where Element: Differentiable & ElementaryFunctions {
  ...
  /// For real types, if `x` is negative the result is `.nan`. For complex
  /// types there is a branch cut on the negative real axis.
  public static func sqrt(_ x: Self) -> Self { .init(x.base.map(Element.sqrt)) }

  /// The cosine of `x`, interpreted as an angle in radians.
  public static func cos(_ x: Self) -> Self { .init(x.base.map(Element.cos)) }

  /// The sine of `x`, interpreted as an angle in radians.
  public static func sin(_ x: Self) -> Self { .init(x.base.map(Element.sin)) }

  /// The tangent of `x`, interpreted as an angle in radians.
  public static func tan(_ x: Self) -> Self { .init(x.base.map(Element.tan)) }

  /// The inverse cosine of `x` in radians.
  public static func acos(_ x: Self) -> Self { .init(x.base.map(Element.acos)) }

  /// The inverse sine of `x` in radians.
  public static func asin(_ x: Self) -> Self { .init(x.base.map(Element.asin)) }

  /// The inverse tangent of `x` in radians.
  public static func atan(_ x: Self) -> Self { .init(x.base.map(Element.atan)) }

  /// The hyperbolic cosine of `x`.
  public static func cosh(_ x: Self) -> Self { .init(x.base.map(Element.cosh)) }

  /// The hyperbolic sine of `x`.
  public static func sinh(_ x: Self) -> Self { .init(x.base.map(Element.sinh)) }

  /// The hyperbolic tangent of `x`.
  public static func tanh(_ x: Self) -> Self { .init(x.base.map(Element.tanh)) }

  /// The inverse hyperbolic cosine of `x`.
  public static func acosh(_ x: Self) -> Self { .init(x.base.map(Element.acosh)) }

  /// The inverse hyperbolic sine of `x`.
  public static func asinh(_ x: Self) -> Self { .init(x.base.map(Element.asinh)) }

  /// The inverse hyperbolic tangent of `x`.
  public static func atanh(_ x: Self) -> Self { .init(x.base.map(Element.atanh)) }

  /// The exponential function applied to `x`, or `e**x`.
  public static func exp(_ x: Self) -> Self { .init(x.base.map(Element.exp)) }
  ...
  /// The natural logarithm of `x`.
  public static func log(_ x: Self) -> Self { .init(x.base.map { Element.exp($0) }) }
  ...
  /// an integral value. For complex types, there is a branch cut on the
  /// negative real axis.
  public static func pow(_ x: Self, _ y: Self) -> Self { .init(zip(x.base, y.base).map(Element.pow)) }

  /// `x` raised to the `n`th power.
  ///
  /// The product of `n` copies of `x`.
  public static func pow(_ x: Self, _ n: Int) -> Self { .init(x.base.map { Element.pow($0, n) }) }

  /// The `n`th root of `x`.
  ///
  /// For real types, if `x` is negative and `n` is even, the result is NaN.
  /// For complex types, there is a branch cut along the negative real axis.
  public static func root(_ x: Self, _ n: Int) -> Self { .init(x.base.map { Element.root($0, n) }) }
}
#endif
...
// VectorProtocol and PointwiseMultiplicative conformances for Array.DifferentiableView
// Only available for non-standard toolchain as _base is internal in Swift 6.3
#if !TENSORFLOW_USE_STANDARD_TOOLCHAIN
extension Array.DifferentiableView: VectorProtocol
where Element: Differentiable & VectorProtocol {
  public typealias VectorSpaceScalar = Element.VectorSpaceScalar

  public func adding(_ x: VectorSpaceScalar) -> Array<Element>.DifferentiableView {
    .init(base.map { $0.adding(x) })
  }
  ...
  public func subtracting(_ x: VectorSpaceScalar) -> Array<Element>.DifferentiableView {
    .init(base.map { $0.subtracting(x) })
  }
